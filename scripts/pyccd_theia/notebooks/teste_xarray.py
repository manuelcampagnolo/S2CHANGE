...
from read_files import read_tif_files, convertPointToCrs, get_most_recent_file
...
df_final.to_csv(base_path / 'outputs' / 'csv' / 'Theia' / f'CCD_{timestamp}_{random_state_value}_alpha_{alpha}.csv', index=False)
...
csv_file_ccd=get_most_recent_file((base_path / 'outputs' / 'csv' / 'Theia'),exclude_string='pre_proc')
...
DF_FINAL_T.to_csv(base_path / 'outputs' / f'Validacao_Theia_{random_state_value}_alpha_{alpha}.csv', index=False)