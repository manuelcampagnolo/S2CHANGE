...
file_path = r'C:\Users\scaetano\Downloads\nvg_2018_ccd_with_tiles.gpkg' # Nome dos tiles nas geometrias da BDR
tile_to_test = 'T29SMD'  # Escolher o tile 
...
if not os.path.exists(file_path):
    print("Arquivo não encontrado. Criando o arquivo com os tiles...")
    ...
    for idx, geom in gdf.iterrows():
        # Verificar os tiles que contêm completamente a geometria
        containing_tiles = tiles_gdf[tiles_gdf.contains(geom.geometry)]
        
        if not containing_tiles.empty:
            # Se houver um tile que contém a geometria completamente, usa esse tile
            tile_id = containing_tiles.iloc[0]['Name']  
            gdf.at[idx, 'tile'] = tile_id
        else:
            # Se não houver um tile que contenha completamente, verificar interseção
            intersecting_tiles = tiles_gdf[tiles_gdf.intersects(geom.geometry)]
            if not intersecting_tiles.empty:
                # Se houver interseções, assume o primeiro tile que intersecta
    ...
    print(f"Arquivo '{file_path}' já existe. Carregando dados...")
...
# Função para filtrar imagens com base em nuvens usando a coleção S2 Cloud Probability
...
# Funçao para obter a coleçao de imagens filtrada
...
# Funçao para adicionar banda de data em milissegundos
...
# # Função para exportar imagem para o Google Drive
# def exportImage(image, i, tile, geometry):
#     try:
#         print('Exporting image: {} for tile: {}'.format(i, tile))

#         dateInMillis = image.date().getInfo()['value']
#         fileName = 'S2SR_image_{}_tile_{}'.format(dateInMillis, tile)
#         image = convert_band_types(image)
#         # Aplicar a geometria como mascara na imagem
#         masked_image = image.clip(geometry.buffer(500)).unmask(65535, False)
...
#             region=geometry.buffer(500)
#         )

#         task.start()

#         print('Export task {} started.'.format(i))
#     except Exception as e:
#         print('Exporting image {} failed with error: {}'.format(i, str(e)))
...
def exportImage(image, i, tile, geometry, folder_name):
    try:
        ...
        fileName = 'S2SR_image_{}_tile_{}'.format(dateInMillis, tile)
        image = convert_band_types(image)
        # Aplicar a geometria como máscara na imagem
        masked_image = image.clip(geometry.buffer(500)).unmask(65535, False)
        ...
        task = ee.batch.Export.image.toDrive(
            ...
            maxPixels=1e13,
            formatOptions={'noData': 65535},
            region=geometry.buffer(500)  # Use a geometria definida aqui
        )
        ...
        print(f'Exporting image {i} failed with error: {str(e)}')
...
def load_geometry_from_geopackage_parts(file_path, tile, half=0.5, part='first'):
    """
    Carrega uma fração das geometrias para o tile especificado e retorna a geometria unida.
    
    Args:
    - file_path (str): Caminho para o arquivo GeoPackage.
    - tile (str): Identificador do tile.
    - half (float): Fração das geometrias a serem carregadas.
    - part (str): Indica se deve carregar a 'primeira' ou 'segunda' metade das geometrias.
    
    Returns:
    - ee.Geometry: Geometria unida correspondente à fração e parte especificadas.
    """
    # Carregar o arquivo GeoPackage
    gdf = gpd.read_file(file_path)
    ...
    # Verificar se o tile existe no GeoDataFrame
    if tile not in gdf['tile'].values:
        raise ValueError(f"Tile {tile} não encontrado no GeoPackage.")
    ...
    if not gdf_tile.empty:
        # Selecionar a fração especificada
        ...
        if part == 'first':
            ...
            raise ValueError("O parâmetro 'part' deve ser 'first' ou 'second'.")
...
# ---------------------------------
try:
    # Verifica se o tile é o especial T29SNB
    if tile_to_test == tile_to_split:
        # Usar divisão de geometria para este tile
    ...
    if geometry:
        ...
        print(f'Geometria para o tile {tile_to_test} não encontrada.')
