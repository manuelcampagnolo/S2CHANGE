...
PASTA_DE_INPUTS = Path(r'C:/Users/Public/Documents/imagens_THEIA/')#Path('D:/')
...
batch_size = 1000 # Número de pixels para cada lote -- to be done sequentially on each core (either hpc or local)
...
def main(batch_size):
    ...
    size = os.cpu_count()
    batches_per_process = [batches[i::size] for i in range(size)]
    #print('num batches = {}'.format(len(batches)))

    #print(batches_per_process)
    args = [(batch, output_file, tif_dates_ord) for batch in batches_per_process]
    ...
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        ...
        for batch_results in executor.map(process_batch, args):
            dfs.extend(batch_results)  # Adiciona os resultados processados; dataframe or list of dataframes
            tqdm_bar.update(1)
        
        tqdm_bar.close()
    print('parallel execution took {}s'.format(round(time.time()-tstart,2)))
    exit()
...
def process_batch(args):
    ...
    """
    batches, sel_values_path, tif_dates_ord = args
    results = []
    with h5py.File(sel_values_path, 'r') as h5_file:
        ...
        for batch in batches:
            tstart = time.time()
            start, end = batch
            sel_values = h5_file['values'][:,:,start:end]
            xs = h5_file['xs'][start:end]
            ys = h5_file['ys'][start:end]
            # sel_values = np.ascontiguousarray(sel_values)
            # xs = np.ascontiguousarray(xs)
            # ys = np.ascontiguousarray(ys)
            arg_list = [(i, sel_values, tif_dates_ord, xs, ys,NODATA_VALUE, MAX_VALUE_NDVI, PASTA_DE_OUTPUTS, CRS_THEIA, CRS_WGS84, img_collection)
                        for i in range(end-start)]
            results.append([runDetectionForPoint(arg) for arg in arg_list])
            count = count+1
            print('count = {}/{}. time per batch = {}s'.format(count, len(batches), round(time.time()-tstart,2)))

    return results

import numpy as np
def process_batch_npy(args):
    """
    Processes a batch of points for change detection.
    ...
    for batch in batches:
        tstart = time.time()
        start, end = batch
        sel_values = np.load(sel_values_path, mmap_mode='r')[:, :, start:end]
        xs = np.load(str(sel_values_path.with_suffix('')) + '_xs.npy', mmap_mode='r')[start:end]
        ys = np.load(str(sel_values_path.with_suffix('')) + '_ys.npy', mmap_mode='r')[start:end]
        arg_list = [(i, sel_values, tif_dates_ord, xs, ys,NODATA_VALUE, MAX_VALUE_NDVI, PASTA_DE_OUTPUTS, CRS_THEIA, CRS_WGS84, img_collection)
                        for i in range(end-start)]
        results.append([runDetectionForPoint(arg) for arg in arg_list])
        count = count+1
...
def process_in_series(args):

    sel_values, tif_dates_ord, xs, ys, N = args
